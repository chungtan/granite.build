...
class BaseSpaceUserStorageTest(AbstractStorageTest):
    ...
    def test_get_by_username_case_insensitive(self):
        storage = self._get_tested_storage()
        item = StoredSpaceUser(
            space_name="ci_space", username="alice@example.com", role="member"
        )
        storage.add(item)

        # Uppercase input matches lowercase stored value
        results = storage.get_by_username("Alice@Example.COM")
        assert len(results) == 1
        assert results[0].username == "alice@example.com"

        # Store with mixed case, lookup with lowercase
        item2 = StoredSpaceUser(
            space_name="ci_space2", username="Bob@Example.COM", role="admin"
        )
        storage.add(item2)

        results2 = storage.get_by_username("bob@example.com")
        assert len(results2) == 1
        assert results2[0].username == "Bob@Example.COM"

    def test_get_by_space_and_username_case_insensitive(self):
        storage = self._get_tested_storage()
        item = StoredSpaceUser(
            space_name="ci_proj", username="alice@example.com", role="admin"
        )
        storage.add(item)

        found = storage.get_by_space_and_username("ci_proj", "Alice@Example.COM")
        assert found is not None
        assert found.role == "admin"

        # Store with mixed case, lookup with lowercase
        item2 = StoredSpaceUser(
            space_name="ci_proj2", username="Bob@Example.COM", role="member"
        )
        storage.add(item2)

        found2 = storage.get_by_space_and_username("ci_proj2", "bob@example.com")
        assert found2 is not None
        assert found2.role == "member"
