...
class RabbitMQBase(MessagingBase):
    ...
    async def publish(
        ...
        if self.settings.per_publish_channel:
            ...
            async def _publish(*a: Any, **kw: Any) -> Any:
                assert self._exchange is not None, "exchange lost during retry"
                return await self._exchange.publish(*a, **kw)

            publish_call = self._with_retries(_publish)
